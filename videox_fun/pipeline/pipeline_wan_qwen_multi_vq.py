...
class WanQwenPipeline_Multi_VQ(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        shift: int = 5,
        vit_fea_list: Optional[List[torch.FloatTensor]] = None,
        ...
        if do_classifier_free_guidance:
            ...
            vit_fea = [torch.stack([torch.zeros_like(vit_fea_list[i]), vit_fea_list[i]], dim=0) for i in range(len(vit_fea_list))]
        else:
            in_prompt_embeds = prompt_embeds
            vit_fea = [vit_fea_list[i].unsqueeze(0) for i in range(len(vit_fea_list))]
