...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.fusion_attn = nn.ModuleList([
            WanSelfAttention(dim, num_heads, window_size, qk_norm, eps) for _ in range(num_levels)
        ])
    ...
    def forward(
        ...
        new_vit_fea_list = []
        for i in range(len(vit_fea_list)):
            vit_fea = vit_fea_list[i].unsqueeze(0)
            vit_fea = apply_dropout(vit_fea, training=self.training)
            vit_fea = self.vit_projection[i](vit_fea)
            vit_fea = self.fusion_attn[i](vit_fea, seq_lens, grid_sizes, self.freqs, dtype, t=t)
            new_vit_fea_list.append(vit_fea)
...
class Wan2_2Transformer3DModel_Multi_VQ(WanTransformer3DModel):
    r"""
    Wan diffusion backbone supporting both text-to-video and image-to-video.
    """
