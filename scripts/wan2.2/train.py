...
from videox_fun.models import (AutoencoderKLWan, AutoencoderKLWan3_8, WanT5EncoderModel,
                              Wan2_2Transformer3DModel, Wan2_2Transformer3DModel_Multi_VQ)
from videox_fun.pipeline import Wan2_2Pipeline, Wan2_2I2VPipeline
from videox_fun.utils.discrete_sampler import DiscreteSampling
from videox_fun.utils.utils import get_image_to_video_latent, save_videos_grid

# qwen3_vl
from qwen3_vl_multi_vq import Qwen3VLForConditionalGeneration
...
def parse_args():
    ...
    parser.add_argument(
        "--lr_warmup_ratio", type=float, default=None, help="Number of warm up ratio for the warmup in the lr scheduler."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Sample stride of the vit input.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=[],
        help="The image resolution list for training.",
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--add_zehui_data",
        action="store_true",
        help="Whether to add zehui data for training.",
    )
    parser.add_argument(
        "--add_zeyuan_data",
        action="store_true",
        help="Whether to add zeyuan data for training.",
    )
    parser.add_argument(
        "--add_llava_video_data",
        action="store_true",
        help="Whether to add llava data for training.",
    )
    parser.add_argument(
        "--add_image_data",
        action="store_true",
        help="Whether to add image data for training.",
    )
    parser.add_argument(
        "--show_data_structure",
        action="store_true",
        help="Whether to show data structure.",
    )
    parser.add_argument(
        "--vq_model_path",
        type=str,
        default=None,
        help="If you want to load the weight from other vq models, input its path.",
    )
...
def main():
    ...
    if args.boundary_type == "low" or args.boundary_type == "full":
        ...
        sub_path = config['transformer_additional_kwargs'].get('transformer_high_noise_model_subpath', 'transformer')
    transformer3d = Wan2_2Transformer3DModel_Multi_VQ.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path, sub_path),
        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
    ).to(weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.no_grad():
                    vit_features = qwen3_vit(batch['vit_values'][0], grid_thw=batch['grid_thw'][0])[0]
