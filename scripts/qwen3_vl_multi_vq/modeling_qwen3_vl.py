...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def _deepstack_process(
        ...
        mask = visual_pos_masks.view(-1, 1, 1).to(hidden_states.device, hidden_states.dtype)
        hidden_states = hidden_states + mask * visual_embeds
        return hidden_states
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        image_embeds, deepstack_image_embeds, code_idx, vq_loss = self.visual(pixel_values, grid_thw=image_grid_thw)
        # print("grid_thw:", image_grid_thw, "embeds shape:", image_embeds.shape)
        split_sizes = (image_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        image_embeds = torch.split(image_embeds, split_sizes)
        return image_embeds, deepstack_image_embeds, code_idx, vq_loss
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if inputs_embeds is None:
            if task_type == "understanding" or len(input_ids[0]) > 1:
                ...
                inputs_embeds_final = self.visual.vq.quantize.dequantize(input_ids[3:4, :]).to(self.language_model.embed_tokens.weight.dtype)
                inputs_embeds = inputs_embeds_final.repeat(4, 1, 1)
        ...
        if pixel_values is not None:
            ...
            vq_loss = vq_loss * self.image_vq_loss_weight
        ...
        if pixel_values_videos is not None:
            video_embeds, deepstack_video_embeds, code_idx, vq_loss = self.get_video_features(pixel_values_videos, video_grid_thw)
            vq_loss = vq_loss * self.video_vq_loss_weight
        ...
        if 'deep_stack_features' in kwargs and kwargs['deep_stack_features'] is not None and len(input_ids[0]) == 1:
            ...
            ds_0 = self.visual.vq.quantize.dequantize(input_ids[0:1, :]).to(self.language_model.embed_tokens.weight.dtype)
            ds_1 = self.visual.vq.quantize.dequantize(input_ids[1:2, :]).to(self.language_model.embed_tokens.weight.dtype)
            ds_2 = self.visual.vq.quantize.dequantize(input_ids[2:3, :]).to(self.language_model.embed_tokens.weight.dtype)
            # deepstack_visual_embeds = torch.cat([ds_0, ds_1, ds_2], dim=0).to(self.language_model.embed_tokens.weight.dtype)
        ...
        if past_key_values.layers[0].keys.shape[0] == 1:
            past_key_values.batch_repeat_interleave(4)
            outputs.last_hidden_state = outputs.last_hidden_state.repeat(4, 1, 1)
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.gen_head_3 = nn.Linear(config.text_config.hidden_size, self.vision_vocab_size, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if task_type == "understanding":
            ...
            logits_0 = self.gen_head_0(hidden_states[0:1, slice_indices, :])
            logits_1 = self.gen_head_1(hidden_states[1:2, slice_indices, :])
            logits_2 = self.gen_head_2(hidden_states[2:3, slice_indices, :])
            logits_3 = self.gen_head_3(hidden_states[3:4, slice_indices, :])
            if labels is not None:
